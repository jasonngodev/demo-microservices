...
public class InventoryService:MongoDbRepository<InventoryEntry>, IInventoryService
{
    ...
    {
        var filterSearch = Builders<InventoryEntry>.Filter.Empty;
        var filterItemNo = Builders<InventoryEntry>.Filter.Eq(x => x.ItemNo, query.ItemNo());
        if (!string.IsNullOrEmpty(query.SearchItem))
            filterSearch = Builders<InventoryEntry>.Filter.Eq(x => x.DocumentNo, query.SearchItem);
        var andFilter = filterItemNo & filterSearch;
